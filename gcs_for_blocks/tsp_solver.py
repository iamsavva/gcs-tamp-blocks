...
class TSPasGCS:
    ...
    def add_decision_variables_to_primal(self):
        ...
        for e in self.edges.values():
            ...
            e.set_left_order(self.primal_prog.NewContinuousVariables(1, "left_order_" + e.name)[0])
            e.set_right_order(
    ...
    def add_constraints_to_primal(self):
        ...
        for e in self.edges.values():
            ...
            if e.left.name == self.start:
                ...
                self.primal_prog.AddLinearConstraint(le(A1 @ np.array([e.left_order, e.phi]), b1))
            elif e.left.name[0] == "t":
                # all target nodes have order of at least 2
                self.primal_prog.AddLinearConstraint(le(A2 @ np.array([e.left_order, e.phi]), b2))
            ...
            if e.right.name[0] == "s":
                # all start nodes have order of at least 1
                self.primal_prog.AddLinearConstraint(le(A1 @ np.array([e.right_order, e.phi]), b1))
            else:
                # all target nodes have order of at least 2
                self.primal_prog.AddLinearConstraint(le(A2 @ np.array([e.right_order, e.phi]), b2))
        ...
        self.primal_prog.AddLinearConstraint(
            sum([e.right_order for e in self.edges.values() if e.right.name in left_vertices])
            == left_order_sum
        )
        self.primal_prog.AddLinearConstraint(
            sum([e.left_order for e in self.edges.values() if e.left.name in left_vertices])
            == left_order_sum
        )
        self.primal_prog.AddLinearConstraint(
            sum([e.right_order for e in self.edges.values() if e.right.name in right_vertices])
            == right_order_sum
        )
        self.primal_prog.AddLinearConstraint(
            sum([e.left_order for e in self.edges.values() if e.left.name in right_vertices])
    ...
    def verbose_solution(self):
        ...
        flow_vars = [(e.name, self.primal_solution.GetSolution(e.phi), e.cost) for e in self.edges.values()]
        ...
        for v in self.vertices.values():
            ...
            sum_of_y = sum([self.primal_solution.GetSolution(self.edges[e].left_order) for e in v.edges_out])
            sum_of_z = sum([self.primal_solution.GetSolution(self.edges[e].right_order) for e in v.edges_in])
